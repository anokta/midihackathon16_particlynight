...
public class InputManager : MonoBehaviour {
  ...
  void Awake () {
    drawSpring = 
      ((GameObject)GameObject.Instantiate(gameManager.springPrefab)).GetComponent<Spring>();
    drawSpring.enabled = false;
    drawSpring.GetComponent<CapsuleCollider>().enabled = false;
  ...
  void Update () {
    ...
    if (Input.GetMouseButtonDown(0)) {
      if (ctrlPressed && objectSelected) {
        ...
        selectedParticle = gameManager.InstantiateParticle(Vector3.zero);
      } else {
        mode = InputMode.None;
      }
    }
    ...
    case InputMode.Spring:
      ...
      if (Input.GetMouseButtonUp(0)) {
        ...
        if (selectedParticle != null) {
          ...
          gameManager.InstantiateSpring(drawSpring.particleA, drawSpring.particleB);
        }
        drawSpring.SetTransform(Vector3.zero, Vector3.zero);
      }
      break;
